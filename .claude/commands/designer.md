  ...
  phases:
    - phase: 1
      ...
      optional_background_research:
        ...
        parallel_fanout:
          when: "2+ INDEPENDENT research areas (affected subsystem, contract surfaces, test landscape, in-repo prior art) — no file overlap, read-only"
          action: "Dispatch ONE background code-researcher PER independent area in a SINGLE message (Agent tool, run_in_background: true). Integrate ALL summaries at the async_integration_point before Phase 3 PROPOSE. See parallel-dispatch.md Use Case 1b."
          fallback: "Areas coupled or single-domain → ONE bundled background researcher (previous single-researcher behavior)."
