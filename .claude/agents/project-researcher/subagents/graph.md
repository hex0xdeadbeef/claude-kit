...
for target in state.discover.analysis_targets:
  ...
  # TRY first — if get_symbols fails with 'captures' error, see fallback below
...
**IMPORTANT — get_symbols / get_dependencies may fail (tree-sitter 0.24+ API issue):**

If `get_symbols` or `get_dependencies` returns `'tree_sitter.Query' object has no attribute 'captures'`:

1. **Do NOT abort.** Fall back to `run_query` + `get_ast` pattern.
2. Update `state.graph.analysis_method = "tree-sitter-mcp-partial"`, confidence_modifier = -0.05
3. Use `run_query` with S-expression patterns from `deps/tree-sitter-patterns.md` instead:

```yaml
# Fallback symbol extraction via run_query (when get_symbols fails):
functions = run_query(
...
structs = run_query(
  project=project_name,
  query="(type_declaration (type_spec name: (type_identifier) @name type: (struct_type)))",
  language="go"
)
interfaces = run_query(
  project=project_name,
  query="(type_declaration (type_spec name: (type_identifier) @name type: (interface_type)))",
  language="go"
)

# Fallback dependency extraction (when get_dependencies fails):
# For Go: use go list
# bash: go list -json ./... 2>/dev/null | jq '{ImportPath, Imports}'
# Or grep: grep -rn '"[^"]*"' --include="*.go" (import lines)
```

**See:** `deps/tree-sitter-patterns.md#known-issues` for full explanation and fix instructions.
