...
1. **STARTUP**
   ...
   - Load your review rules: Read `.claude/skills/code-review-rules/SKILL.md` in full — it is your severity/decision/auto-escalation rubric + grep search_patterns + on-demand supporting-file index. It is NOT preloaded (the skill is `disable-model-invocation: true`, which blocks subagent preload), so you MUST Read it here. Plugin mode: if a BUNDLED KIT ROOT directive is present in your injected context (additionalContext, the delegation prompt, or the worktree sidecar above), resolve this path under that root (the skill ships in the plugin, not the project).
     - Trusted-anchor rule (no filesystem search): resolve `code-review-rules/SKILL.md` ONLY from a trusted anchor — either (a) the project-relative path `.claude/skills/code-review-rules/SKILL.md` in a project-scoped install, or (b) under an injected BUNDLED KIT ROOT directive in plugin mode. If NEITHER resolves, you MUST NOT search the filesystem (no `find`, no glob, no disk-wide scan) for an arbitrary copy — a sibling checkout may carry a version-unpinned or locally-modified rubric and silently change your verdict. Instead STOP and emit `VERDICT: REJECTED` with this setup-error note in the handoff narrative: "Rubric unresolvable — code-review-rules/SKILL.md not found under a trusted anchor (project path or BUNDLED KIT ROOT). Review aborted to avoid loading an arbitrary on-disk rubric." REJECTED triggers a workflow STOP (not a coder retry) — a missing rubric is a setup error, not a code defect.
     - Rubric provenance (report + warn): after loading `code-review-rules/SKILL.md`, emit one line — `Rubric loaded from: <absolute path>`. If a BUNDLED KIT ROOT directive was present AND the loaded path is NOT under that root, additionally emit `⚠️ WARN: rubric loaded from outside the active BUNDLED KIT ROOT — possible version drift`. This is a provenance signal in your narrative only; it does NOT change the `VERDICT:` line or the `VERDICT_JSON:` block.
