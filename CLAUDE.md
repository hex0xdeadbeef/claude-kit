...
- `CLAUDE_KIT_PHASE_COMPLETION_NOTIFY` — Phase-5 completion desktop notification (Claude Code v2.1.141+). Default `off` (skip notification). Set to `on` to emit an OSC 9 desktop notification via `terminalSequence` JSON output when the latest workflow checkpoint shows `phase_completed: 5` AND verdict in {APPROVED, APPROVED_WITH_COMMENTS}. Allowlisted escape sequences only (OSC 0/1/2/9/99/777 + BEL); CSI / OSC 8 / OSC 52 / OSC 1337 are rejected. Disable on headless CI to avoid notification noise.
- `CLAUDE_KIT_MCP_PRELOAD` — opt-in flag for MCP `sequential-thinking` pre-loading (Claude Code v2.1.121+). Default `off` (keeps deferred ToolSearch behavior). Set to `on` to instruct the kit to expect `.mcp.json` to declare `alwaysLoad: true` on `sequential-thinking`; `mcp-preload-warn.sh` emits a `WARN` at SessionStart **only when an active workflow checkpoint is present** under `.claude/workflow-state/` if the env is on but the config is missing the opt-in (drift detection scoped to pipeline contexts, per PR-008 gate). Recommended only for XL-heavy workflows.
...
**Phase 0.7 is invocable, exclusive, and gated.** `/designer` is invoked with the Skill tool by name (`designer`; plugin mode `claude-kit:designer`) — it is a command, so agent auto-delegation never applies to it. The orchestrator never performs the design phase itself: no self-written draft, no ad-hoc critic subagents (Phase 3.5 is 7 in-context lenses, zero subagents). Two `rules:` entries in `workflow.md` state this — `Named executor` and `Artifacts are evidence, not doctrine` (prior `.claude/prompts/` files record what past runs did, including their improvisations; they are never doctrine). `pipeline.spec_gate` runs before Phase 1 on every run — deliberately outside `designer_delegation`, because `post_delegation` cannot police a delegation that never happened — and a missing or unapproved spec on the L/XL route is FATAL. Branch order is load-bearing and declared `FIRST MATCH WINS`: a usable approved spec is handed on first (so a re-routed re-plan never loses its `--spec`), then the exemptions for runs that legitimately have no spec — a re-route (`re_routing.occurred` with `re_routing.phase` in `plan-review`/`implementation`, NOT `original_route`, since the kit has no canonical complexity→route mapping) and an explicit `design_waived: true` — then the FATAL branches. Spec staleness is a WARN keyed on `spec_artifact`, never a reclassification to missing: `phase_completed` is a single scalar Phase 1 overwrites, so treating it as proof would FATAL every `--from-phase 1` resume. Standalone `/planner` WARNs rather than aborting. Verification: `.claude/scripts/tests/test-designer-phase-invocation.sh` (23 checks; branch order and every exemption are mutation-tested).
