...
| `uvx`              | uv — `curl -LsSf https://astral.sh/uv/install.sh \| sh` (macOS/Linux) / `irm https://astral.sh/uv/install.ps1 \| iex` (Windows) | `.mcp.json` runtime for `tree_sitter` MCP server (`uvx --python python3.11 mcp-server-tree-sitter`) |
